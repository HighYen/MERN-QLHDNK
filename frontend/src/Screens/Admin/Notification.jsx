...
const Material = () => {
  ...
  const [notice, setSubject] = useState();
    ...
    axios
      ...
      .then((response) => {
        ...
        if (response.data.success) {
          setSubject(response.data.notice); // Đổi thành notices
        } else {
      ...
      <div className="w-full flex justify-evenly items-center mt-12">
        <div className="w-1/2 flex flex-col justify-center items-center">
          <div className="w-[80%] mt-2">
            ...
            <textarea
              id="title"
              className="bg-blue-50 py-2 px-4 w-full mt-1 rounded-sm"
              value={selected.title}
              onChange={(e) =>
                setSelected({ ...selected, title: e.target.value })
              }
              rows={10} // Số dòng hiển thị ban đầu
